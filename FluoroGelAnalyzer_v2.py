...
class DensitometryApp:
    def __init__(self, root: tk.Tk) -> None:
        """Initialize the densitometry app and its tracking state."""
        self.root = root
    ...
    def _build_ui(self) -> None:
        """Build the main toolbar, controls, and plotting layout."""
    ...
    def save_session(self) -> None:
        """Save the current image, ROI, marker, and AUC state to a session file."""
    ...
    def open_session(self) -> None:
        """Restore a saved analysis session and redraw all ROIs and plots."""
        path = filedialog.askopenfilename(
    ...
    def apply_rotation(self) -> None:
        """Rotate the original image by the selected angle and redraw the view."""
    ...
    def reset_rotation(self) -> None:
        """Reset the image back to its unrotated state."""
        if self.original_image is None:
            return
    ...
    def enable_crop(self) -> None:
        """Activate the crop tool so the visible image can be narrowed to a region of interest."""
        if self.image is None:
            messagebox.showinfo("Open TIFF", "Open a TIFF image first.")
            return
    ...
    def _crop_selected(self, click, release) -> None:
        """Apply the crop rectangle chosen by the user to the displayed image."""
    ...
    def reset_image_view(self) -> None:
        """Restore the full image view to the original frame and limits."""
        if self.image is None:
            return
        self._disconnect_selectors()
    ...
    def enable_vertical_line(self) -> None:
        """Let the user add vertical boundary markers on the lane profile plot."""
    ...
    def _vertical_line_click(self, event) -> None:
        """Add a vertical marker at the clicked x-position on each lane profile."""
    ...
    def enable_roi(self) -> None:
        """Start drawing a rectangular ROI on the image to define a lane."""
        if self.image is None:
            messagebox.showinfo("Open TIFF", "Open a TIFF image first.")
            return
        self._disconnect_selectors()
    ...
    def enable_copy_roi(self) -> None:
        """Create a second ROI copied 170 pixels above the current lane for comparison."""
    ...
    def _roi_selected(self, click, release) -> None:
        """Finalize a dragged ROI rectangle and build the profile from its pixels."""
        if self.image is None:
            return
    ...
    def _nudge_roi(self, dx: int, dy: int) -> None:
        """Move the active ROI by a small step when the arrow keys are used."""
        if self.image is None or self.roi_bounds is None:
            return
        x0, y0, width, height = self.roi_bounds
        self._set_roi(
    ...
    def _arrow_key_move_roi(self, event):
        """Handle arrow-key nudging so the current ROI can be repositioned precisely."""
    ...
    def _delete_key_remove_roi(self, event):
        """Delete the active ROI when the Delete key is pressed."""
        if event.widget.winfo_class() in {"Entry", "TEntry", "TSpinbox"}:
            return
    ...
    def _delete_active_roi(self) -> None:
        """Remove the selected ROI and all of its associated profile annotations."""
    ...
    def _set_roi(
        ...
        """Draw or update an ROI rectangle and rebuild the profile data for that lane."""
        if self.image is None:
            return
    ...
    def _schedule_profile_update(self) -> None:
        """Delay profile recalculation briefly after dragging so the plot stays responsive."""
    ...
    def _apply_delayed_profile_update(self) -> None:
        """Apply the queued ROI profile refresh after the drag finishes."""
        self.profile_update_after_id = None
        if self.roi_bounds is not None:
            self._update_profile_plot()

    def _update_profile_plot(self) -> None:
        """Compute the lane profile from the ROI and redraw the plot with axes and labels."""
        if self.image is None or self.roi_bounds is None:
            return
        x0, y0, width, height = self.roi_bounds
        x1, y1 = x0 + width, y0 + height
    ...
    def connect_marker_background(self) -> None:
        """Connect two vertical markers into a baseline curve for background subtraction."""
        marker_groups = {}
    ...
    def _add_profile_axis(self) -> None:
        """Create a new profile subplot for an additional ROI lane."""
    ...
    def _align_profile_x_axes(self) -> None:
        """Keep all profile plots on the same x-axis range for consistent lane comparisons."""
    ...
    def _layout_profile_axes(self) -> None:
        """Arrange the ROI profile subplots and summary panel in the figure layout."""
    ...
    def _update_profile_auc_text(self, axis) -> None:
        """Refresh the on-plot AUC label for a specific lane profile."""
    ...
    def _update_marker_summary(self) -> None:
        """Summarize the current lane boundaries, positions, and AUC results in the side panel."""
    ...
    def enable_background(self) -> None:
        """Allow the user to define a baseline curve for background subtraction."""
        if self.profile is None:
            messagebox.showinfo("Select ROI", "Select a lane ROI first.")
            return
        self._disconnect_selectors()
    ...
    def _background_click(self, event) -> None:
        """Record clicked baseline points to build the background line for the profile."""
    ...
    def _draw_baseline(self) -> None:
        """Interpolate the selected baseline points into a continuous background curve."""
    ...
    def _profile_for_axis(self, axis):
        """Return the plotted profile data for a specific lane axis."""
        return self.profile_data_by_axis.get(axis)

    def _compute_auc(self, axis, peak: tuple[float, float], show_popup: bool = True):
        """Compute the background-subtracted area under a selected peak region."""
    ...
    def _draw_corrected_auc(self, peak: tuple[float, float]) -> None:
        """Measure the corrected AUC for an identified peak region without a popup."""
        self._compute_auc(self.profile_axis, peak, show_popup=False)

    def enable_wand(self) -> None:
        """Activate the wand tool for selecting enclosed AUC regions on a profile."""
        if self.profile is None:
            messagebox.showinfo("Select ROI", "Select a lane ROI first.")
            return
        self._disconnect_selectors()
    ...
    def _compute_all_auc_regions(self) -> None:
        """Measure all enclosed AUC regions between consecutive vertical markers."""
        marker_groups = {}
        for line, _ in self.plot_marker_lines:
            marker_groups.setdefault(line.axes, []).append(line)
        results = []
    ...
    def _show_scrollable_auc_results(self, content: str) -> None:
        """Display AUC results in a scrollable popup window for review."""
    ...
    def _auc_click(self, event) -> None:
        """Use a click inside the region to calculate the AUC for that enclosed area."""
    ...
    def _peak_selected(self, xmin: float, xmax: float) -> None:
        """Highlight a selected peak interval and compute its corrected AUC."""
        if xmax - xmin < 1:
            return
    ...
    def undo(self) -> None:
        """Reverse the most recent drawing or measurement action."""
    ...
    def _disconnect_selectors(self) -> None:
        """Disable active drawing selectors and disconnect their event callbacks."""
    ...
    def _update_display(self, _value=None) -> None:
        """Apply the current brightness and contrast settings to the displayed image."""
    ...
    def _display_reference_range(self) -> tuple[float, float]:
        """Compute the percentile-based grayscale range used for display normalization."""
        if self.image is None:
            return 0.0, 1.0
    ...
    def reset_display_range(self) -> None:
        """Reset the display contrast and brightness back to the default window."""
    ...
    def reset(self, clear_image: bool = True) -> None:
        """Clear analysis state and reset the figure to a fresh, empty densitometry session."""
        if self.profile_update_after_id is not None:
            self.root.after_cancel(self.profile_update_after_id)
            self.profile_update_after_id = None
        self._disconnect_selectors()
    ...
    def export_csv(self) -> None:
        """Export the selected profile data as a CSV file for downstream analysis."""
        profile_items = [
    ...
    def export_auc_csv(self) -> None:
        """Export the accumulated AUC measurements to a CSV file."""
...
def main() -> None:
    """Launch the Tkinter app and start the densitometry interface."""
    root = tk.Tk()
